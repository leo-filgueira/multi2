...
titanic_train <- dados[train_indices, ] #Subconjunto de treino

titanic_test <- dados[-train_indices, ] #Subconjunto Teste (Dados excluindo o treino)
```
...
- Regressão Logística
    ```{r}
    head(predict(modelolog, titanic_test))
...
confusionMatrix(data =  as.numeric(class_predictionGLM>0.5), reference=titanic_test$survived) 
```
